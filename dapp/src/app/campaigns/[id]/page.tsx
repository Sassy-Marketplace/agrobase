...
const Campaign: React.FC = ({params}: { params: {id: string} }) => {
    const campaign = campaigns.find((campaign: ICampaign) => campaign.id === +params.id);
            ...
            <div className="bg-[#042B2B] flex flex-col pb-8 pt-10 w-full justify-between items-center">
                <div className="w-10/12 flex flex-col md:flex-row justify-between mb-[5.5rem]">
                    ...
                    <div className="md:hidden">
                        <h1 className={`block text-white text-4xl font-semibold pb-4 ${work.className}`}>{campaign?.name}</h1>
                        <p className={`block text-gray-400 text-[1rem] pb-5 ${work.className}`}>{campaign?.location}</p>
                        ...
                        <section className="flex flex-col justify-between w-full md:w-1/3 lg:w-1/4 bg-[#2B2B2B] p-6 rounded-xl shadow-lg h-[13rem] w-[10rem] flex flex-col gap-3 mb-6">
                                ...
                                <div>
                                    <p className={`text-gray-400 text-[0.9rem] ${space.className}`}>Min price</p>
                                    <h1 className={`text-white pl-1 text-[2.2rem] ${space.className}`}>{campaign?.price}</h1>
                                </div>

                            <Button className={`w-full text-center text-black text-[1rem] py-6 rounded-[3rem] bg-[#03ED0E] ${lato.className}`}>Join Campaign</Button>
                        </section>
                    </div>

                    <section className="flex flex-col w-full md:w-1/2 lg:w-3/5">
                        <h1 className={`hidden md:inline text-white text-4xl font-semibold pb-4 ${work.className}`}>{campaign?.name}</h1>
                        <p className={`hidden md:inline text-gray-400 text-[1rem] pb-5 ${work.className}`}>{campaign?.location}</p>
                ...
                <section className="w-full flex flex-col items-center md:mb-0 mb-[3.5rem]">
                    <div className="w-10/12 flex flex-col md:mb-[3rem]">
                        <div className="mb-[0.5rem] mb-5 flex items-center justify-between md:flex-row flex-col">
                            <h1 className={`text-white text-4xl font-semibold  text-center md:text-left ${work.className}`}>More Campaigns From This Farmer</h1>
                                </div>
